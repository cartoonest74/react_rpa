...
export const Art = ({ book }) => {
  ...
  return (
    <div className='mr-3 inline w-64 min-w-[250px] flex-shrink-0 rounded bg-purple-200 p-1.5'>
      <div className=''>
        <div className='flex items-center justify-between text-xl font-bold text-slate-700'>
          <h3 className='truncate text-slate-600'>{book?.title}</h3>
          <button
        ...
        {book?.id === 0 || isEditing ? (
          ...
          book?.marks
            ...
            .map((mark) => <Frame key={mark.id} book={book} mark={mark} />)
        ) : (
      ...
      <button
        ...
        className='ease_in float-right mt-2 rounded-full bg-purple-500 px-4 py-1 font-medium text-white duration-300 hover:bg-purple-700'
      >
        + Add Mark
      </button>
    </div>
